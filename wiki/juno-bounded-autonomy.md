...
| `maxAutoQueueMissions` | 2 | 自动 bootstrap 新 Mission |
...
| `allowedMissionIds` | self-iterate*, agi-literature, axiom-book, hardening, von-neumann, daily-autonomy | 白名单 |

状态文件：

- `AgentWorkbench/state/bounded-autonomy.json` — 日限额计数
- `AgentWorkbench/state/mission-planner.json` — 最近一次 planner 决策快照
- `AgentWorkbench/state/evolution-fitness.json` — Von Neumann 适应度（见 [juno-von-neumann-unit.md](./juno-von-neumann-unit.md)）
...
flowchart TD
  ...
  G --> REC[recordAutonomyState + evolution fitness]
```
