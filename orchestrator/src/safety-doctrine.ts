/**
 * Destructive-operation guardrails for Juno Overseer agent runs.
 * Used in prompt injection; mirrored in .cursor/hooks/safety-gate-core.mjs for shell hooks.
 */

export type ShellSafetyVerdict = {
  blocked: boolean;
  reason?: string;
  category?: "recursive_delete" | "git_destroy" | "protected_root" | "broad_wildcard";
};
...
function hitsProtectedRoot(token: string): boolean {
...
/** Classify a shell one-liner before execution (hook + tests). */
export function classifyShellCommand(command: string): ShellSafetyVerdict {
...
export function getSafetyDoctrineExcerpt(): string {
  return [
    "## Destructive ops firewall (mandatory)",
    "",
    "- **Never** `rmdir /s /q`, `rm -rf`, `Remove-Item -Recurse -Force` on repo roots or parent folders.",
    "- C:\\...\\Juno Oversight and D:\\DesktopData\\...\\Juno Oversight may be the **same directory** (hard link). Deleting one path deletes everything.",
    "- **Never** `git clean -fdx`, `git reset --hard`, or `git push --force` unless a human explicitly requested it in this slot.",
    "- Prefer `git status`, scoped file edits, and `pnpm clean` (project script) over manual tree deletion.",
    "- To remove duplicates: use `fsutil hardlink list` / compare paths — **do not delete** until confirmed separate copies.",
    "- Violations → REVIEW_VERDICT `BLOCK` + stop work.",
  ].join("\n");
}
